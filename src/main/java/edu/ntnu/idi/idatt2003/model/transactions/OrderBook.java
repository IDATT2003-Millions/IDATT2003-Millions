 ...
 *
 * <p>Call {@link #executeOrders(Exchange, Player)} after each week advance to trigger
 * orders whose price conditions are now satisfied.</p>
 */
public class OrderBook {
  ...
  /**
   * Checks every pending order against current market prices and executes those
   * whose conditions are met.
   *
   * <p>BUY  — executes when currentPrice ≤ targetPrice (and player has sufficient funds).<br>
   * SELL — executes when currentPrice ≥ targetPrice (and the share is still in the portfolio).
   * If the share has already been sold, the order is auto-cancelled.</p>
   */
      ...
      if (order.getType() == OrderType.BUY) {
        if (currentPrice.compareTo(order.getTargetPrice()) <= 0) {
          try {
            ...
            // Insufficient funds — leave pending
          }
        }
      } else { // SELL
        BigDecimal owned = player.getPortfolio().getShares(order.getSymbol()).stream()
            .map(s -> s.getQuantity())
