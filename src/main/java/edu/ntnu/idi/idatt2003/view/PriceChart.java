...
/**
 * A resizable Pane that renders a price-history line chart on an internal Canvas.
 * Call {@link #setData} to push fresh prices; the chart redraws automatically.
 * Hovering near a data point shows a tooltip with the week number and formatted price.
 */
public class PriceChart extends Pane {
  ...
  public PriceChart(List<BigDecimal> initialPrices, double height) {
    ...
    tip.setStyle("-fx-background-color: #162030; -fx-text-fill: #e2e8f0;"
  ...
  private void redraw() {
    ...
    gc.setLineWidth(1);
    for (int i = 1; i < 4; i++) gc.strokeLine(pad, pad + ch * i / 4, w - pad, pad + ch * i / 4);
    ...
    areaXs[n] = xs[n - 1]; areaYs[n] = pad + ch;
    areaXs[n + 1] = xs[0]; areaYs[n + 1] = pad + ch;
