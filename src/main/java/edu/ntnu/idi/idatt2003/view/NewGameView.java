...
/**
 * Setup screen shown before a new game begins. Collects username, starting money, and a stock CSV
 * file.
 */
public class NewGameView {

  /**
   * Holds the data collected from the new-game setup form.
   *
   * @param username the player's chosen name
   * @param money the starting capital
   * @param csvPath path to the CSV file containing the stock listings
   */
  public record NewGameData(String username, BigDecimal money, Path csvPath) {}

  /**
   * Builds and returns the new-game setup scene.
   *
   * @param stage the primary stage (used for the file chooser dialog)
   * @param onStartGame called with the collected form data when the player clicks Start Game
   * @return the configured new-game scene
   */
