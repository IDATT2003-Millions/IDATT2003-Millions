...
public class TransactionsView {
  ...
  private Tab buildPendingTab(List<LimitOrder> pending, Consumer<LimitOrder> onCancel) {
    ...
    TableView<LimitOrder> tv = buildOrderTable(items, false, order -> {
  ...
  private Tab buildCompletedTab(List<LimitOrder> completed) {
    TableView<LimitOrder> tv = buildOrderTable(
        FXCollections.observableArrayList(completed), true, null);
  ...
  private TableView<LimitOrder> buildOrderTable(
    ...
    weekCol.setCellValueFactory(d ->
        new SimpleStringProperty(String.valueOf(d.getValue().getWeekPlaced())));
    weekCol.setMaxWidth(70);
    ...
    qtyCol.setCellValueFactory(d ->
        new SimpleStringProperty(d.getValue().getQuantity().toPlainString()));
    qtyCol.setMaxWidth(100);

    TableColumn<LimitOrder, String> targetCol = new TableColumn<>("Target Price");
    targetCol.setCellValueFactory(d ->
        new SimpleStringProperty(fmt(d.getValue().getTargetPrice())));
      ...
      statusCol.setCellFactory(col -> new TableCell<>() {
        ...
        protected void updateItem(String item, boolean empty) {
          ...
          if (!empty && item != null) {
            ...
            getStyleClass().add(item.startsWith("Executed") ? "positive-change" : "negative-change");
          }
        }
      });
